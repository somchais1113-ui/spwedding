...
const record={submissionId:id,name:'คุณทดสอบ',team:'team_cap',attendance:'attending',partySize:2};
...
test('RSVP accepts exact totals and decline=0, rejects malformed input',()=>{
  ...
  for(const change of [{name:' '},{name:'a'.repeat(81)},{team:'unknown'},{partySize:'6+'},{partySize:1.5},{partySize:0},{partySize:1000},{attendance:'declined'},{submissionId:'bad'},{website:'spam'}])assert.equal(validRecord({...record,...change}),null);
});
...
function gasFixture(){
  ...
  const sheet={
    ...
    insertColumnAfter(index){rows.forEach(r=>r.splice(index,0,''));}
  };
  const context={SpreadsheetApp:{openById:()=>({getSheetByName:()=>sheet,insertSheet:()=>sheet}),flush(){}},PropertiesService:{getScriptProperties:()=>({getProperty:()=>secret})},LockService:{getScriptLock:()=>({waitLock(){locked=true;},tryLock(){if(locked)return false;locked=true;return true;},hasLock:()=>locked,releaseLock(){locked=false;}})},ContentService:{MimeType:{JSON:'json'},createTextOutput:text=>({setMimeType:()=>JSON.parse(text)})}};
  ...
  return{rows,send:body=>context.doPost({postData:{contents:JSON.stringify({...body,secret})}}),fail:()=>fail=true,setup:()=>context.setupRsvp()};
}
test('Google writer deduplicates retries and updates the same response, including team and decline',()=>{
  ...
  assert.equal(gas.send({...record,partySize:8,name:'=1+1',team:'team_bird'}).saved,true);assert.equal(gas.rows[1][3],"'=1+1");assert.equal(gas.rows[1][4],'ทีมพี่เบิร์ด');assert.equal(gas.rows[1][6],8);
  gas.send({...record,attendance:'declined',partySize:0});assert.equal(gas.rows.length,2);assert.equal(gas.rows[1][5],'ไม่สะดวกมาร่วมงาน');assert.equal(gas.rows[1][6],0);
});
test('Google writer migrates an existing v1 header row and preserves unexpected existing sheet',()=>{
  const gas=gasFixture();
  gas.rows.push(['รหัสคำตอบ','บันทึกครั้งแรก','อัปเดตล่าสุด','ชื่อผู้ตอบ','สถานะการเข้าร่วม','จำนวนผู้ร่วมงาน']);
  gas.setup();
  assert.equal(gas.rows[0][4],'ทีม');
  const bad=gasFixture();bad.rows.push(['existing user data']);assert.equal(bad.send(record).saved,false);assert.deepEqual(bad.rows,[['existing user data']]);
