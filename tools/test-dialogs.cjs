...
test('Closing modal restores scroll, inline styles/priorities, focus and original page width',()=>{
 ...
 assert.equal(s.body.style.getPropertyValue('top'),'');assert.equal(s.body.style.getPropertyValue('position'),'');assert.equal(s.body.style.getPropertyValue('width'),'');assert.equal(s.html.style.getPropertyValue('overflow'),'hidden');
...
test('Rapid repeated open/close and queued close events never strand the page lock',()=>{
 ...
 for(let i=0;i<20;i++){s.open(d,s.trigger);d.close();s.open(d,s.trigger);d.flush();assert.equal(s.html.style.getPropertyValue('overflow'),'hidden');assert.equal(s.body.style.getPropertyValue('position'),'');d.close();d.flush();assert.equal(s.before(),s.initial)}
...
test('All modal entrypoints use shared lifecycle and QR file is packaged untransformed',()=>{
 ...
 const html=fs.readFileSync(path.join(web,'index.html'),'utf8');assert.ok(!html.includes('id="download-qr"'));assert.ok(html.includes('data-open-qr aria-haspopup="dialog"'));assert.ok(html.includes('id="open-qr" role="button" tabindex="0"'));assert.ok(html.includes('welcome-mesh'));
});
