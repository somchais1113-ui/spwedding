...
function validRecord(b){
 ...
 if(b.action==='enqueue'){
  try{const source=normalize(b.source);if(b.website||createHash('sha256').update(JSON.stringify(source)).digest('hex')!==b.fingerprint)return null;return {action:b.action,id:b.id,fingerprint:b.fingerprint,source};}catch{return null;}
 }
 if(b.action==='begin'){
...
async function handler(req,res,{env=process.env,fetchImpl=fetch}={}){
 ...
 try{
  ...
  if(!['begin','enqueue'].includes(record.action)&&!timingSafeEqual(Buffer.from(token,'hex'),Buffer.from(body.token,'hex')))return send(403,{error:'token'});
  ...
  if(result.ok!==true||result.id!==record.id){
   const code=['conflict','not_found','fields','limit','image_invalid','queue_not_ready'].includes(result.error)?result.error:'save_failed';
   return send(code==='conflict'?409:code==='limit'?429:502,{error:code});
  }
  if(record.action==='enqueue'&&result.accepted!==true)return send(503,{error:'queue_not_ready'});
  res.setHeader('Server-Timing','google;dur='+(Date.now()-started));
  // Never return Drive links, names, messages or Google credentials to anonymous callers.
  return send(200,{ok:true,id:record.id,accepted:result.accepted===true,saved:result.saved===true,card:result.card===true,ink:result.ink===true,fileComplete:result.fileComplete===true,...(record.action==='begin'?{token}: {})});
