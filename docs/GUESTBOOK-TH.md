...
Canvas 1200 × 720 แสดงผลอัตราส่วน 5:3 พิกัดเส้นเป็นสัดส่วน 0–1 จึงไม่หายเมื่อเปลี่ยนแนวหน้าจอ ใช้ Pointer Events และ pointer capture รองรับยกเลิกการลาก ไม่ดัก touch scroll ภายนอก canvas PNG ส่งออกเป็น 2400×3000 หรือ 3200×1800 พร้อม 300 DPI ตาม docs/WISH-EXPORT-TH.md
...
## 2.3 — ปุ่มส่งความยินดีและวิธีเข้าถึงข้อมูล

ปุ่ม “ส่งความยินดี” ใต้การ์ดเชิญเดิมเพียงเปลี่ยนข้อความขอบคุณ ไม่มีการบันทึก ในฉบับนี้ปุ่มพาไปแบบฟอร์มคำอวยพรเพื่อพิมพ์/เขียนแล้วกด “ส่งคำอวยพร” การกดปุ่มแรกไม่สร้างรายการและไม่แสดงว่าส่งแล้ว

ถ้าเปิดด้วย server.mjs และ POST สำเร็จ ข้อมูลอยู่ที่ `private-wishes/<UUID>.json` ข้าง server.mjs หรือโฟลเดอร์ใน WISHES_DATA_DIR ที่เจ้าของโฮสต์ตั้งไว้ หนึ่งไฟล์ต่อคำอวยพร มี id, createdAt, name, mode และ text (ตัวพิมพ์) หรือ image (PNG แบบ data URL สำหรับลายมือ) ดาวน์โหลดโฟลเดอร์นี้ผ่านระบบจัดการไฟล์ของโฮสต์/SSH แล้วเปิด JSON ด้วยโปรแกรมข้อความเพื่ออ่านข้อความ หรือนำค่า image ไปแปลงเป็น PNG โฟลเดอร์นี้ไม่ได้เปิดอ่านผ่านหน้าเว็บ และไม่มีหน้า admin ในแพ็กเกจ

หากนำเฉพาะ HTML/CSS/JS ขึ้น Vercel แบบ static จะยังไม่มี API ที่เก็บข้อมูลส่วนกลาง ต้องเชื่อมฐานข้อมูล/ที่เก็บไฟล์กับ POST /api/wishes ให้ครบก่อน ส่วน server.mjs ที่แนบเป็นทางเลือกสำหรับ Node server ที่มี persistent disk ไม่ใช่ Vercel Function ที่นำไปวางแล้วใช้ local disk ถาวรได้เลย งานรอบนี้แก้แพ็กเกจ ไม่ได้เชื่อมบัญชีฐานข้อมูลหรืออัปเดตเว็บออนไลน์

localStorage ในเครื่องแขกเก็บเพียงร่าง ไม่ใช่กล่องคำอวยพรของบ่าวสาว ส่วนดาวน์โหลด PNG อยู่ในเครื่องแขกและ Native Share Sheet ส่งต่อไปยังแอปที่แขกเลือก ทั้งสองอย่างไม่ได้บันทึกเข้าส่วนกลางอัตโนมัติ
